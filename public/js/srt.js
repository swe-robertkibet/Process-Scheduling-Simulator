document.getElementById("srtForm").addEventListener("submit", function (e) {
    ...
    // Perform SRT simulation and calculate results
    ...
    while (completedProcesses < numProcesses) {
        ...
        if (remainingTimes[shortestProcessIndex] === 0) {
            completedProcesses++;
            completionTimes[shortestProcessIndex] = currentTime;
            turnaroundTimes[shortestProcessIndex] = completionTimes[shortestProcessIndex] - arrivalTimes[shortestProcessIndex];
            waitingTimes[shortestProcessIndex] = turnaroundTimes[shortestProcessIndex] - burstTimes[shortestProcessIndex];
        }
    }
    ...
    // Explain the results
    explainSRTResults(completionTimes, turnaroundTimes, waitingTimes, averageTurnaroundTime, averageWaitingTime);
...
function explainSRTResults(completionTimes, turnaroundTimes, waitingTimes, avgTurnaroundTime, avgWaitingTime) {
    var explanation = "<h2>Explanation of Shortest Remaining Time (SRT) Scheduling Results</h2><br>";
    ...
    explanation += "<br><strong>Waiting Times:</strong> Waiting time for each process is calculated as the difference between turnaround time and burst time. It indicates the time a process spends waiting in the ready queue before its execution starts:<br>";
